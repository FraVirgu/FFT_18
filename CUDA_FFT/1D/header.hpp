...
void gpu_fft(int grid_size, int log_n, cuDoubleComplex *a, cuDoubleComplex *y, cuDoubleComplex *x, cuDoubleComplex *t, int *atomic_array, int *first_computation_j)
{
    ...
    bool direct = true;
    parallel_fft_first_computation<<<dimGrid, dimBlock>>>(direct, first_computation_j, grid_size, N, grid_size, atomic_array, a, y, x, t, log_n);
    ...
    {
        parallel_fft_second_computation<<<dimGrid, dimBlock>>>(direct, j, grid_size, N, grid_size, atomic_array, y, x, t, log_n);
        cudaDeviceSynchronize();
    }
}
...
// id direct = true -> FFT
// id direct = false -> FFT and IFFT

std::vector<std::complex<double>> kernel(bool direct, const std::vector<std::complex<double>> input)
{
    ...
    else
    {
        ...
        auto end_inverse_parallel = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> duration_inverse_parallel = end_inverse_parallel - end_parallel;
        std::cout << "Parallel FFT + IFFT execution time: " << duration_inverse_parallel.count() << " seconds" << std::endl;
